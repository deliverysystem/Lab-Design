...
void start()
{
	...
	rider[0].exist=1;
	rider[1].exist=0;
	rider[0].changeposi(22,38);		//x,y具体待补充 ,rider[i].A.changeposi(x,y);
	...
	for(;value==1;sysclock++){	 			//大循环，控制整个进程 
		...
		for(j=0;j<size;j++){				
			if(((Menu[j].x1-3)==(Menu[j].p)->x&&Menu[j].y1==(Menu[j].p)->y)||(Menu[j].x1==(Menu[j].p)->x&&(Menu[j].y1+5)==(Menu[j].p)->y)||(Menu[j].x1==(Menu[j].p)->x&&(Menu[j].y1-4)==(Menu[j].p)->y)||((Menu[j].x1+3)==(Menu[j].p)->x&&(Menu[j].y1)==(Menu[j].p)->y)&&Menu[j].get==0){ //骑手到达接餐地 
				Menu[j].get=1;
			}
			if(((Menu[j].x2-3)==(Menu[j].p)->x&&Menu[j].y2==(Menu[j].p)->y)||(Menu[j].x2==(Menu[j].p)->x&&(Menu[j].y2+5)==(Menu[j].p)->y)||(Menu[j].x2==(Menu[j].p)->x&&(Menu[j].y2-4)==(Menu[j].p)->y)||((Menu[j].x2+3)==(Menu[j].p)->x&&(Menu[j].y2)==(Menu[j].p)->y)&&Menu[j].get==1){	//骑手到达送餐地 
